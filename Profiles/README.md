...
There are three primary Dispatcher profiles in the `Profiles` directory:
 ...
 - `Dispatcher Current` - This is the one recommended for general use on the layout.
...
Finally, sign into the `dispatch` account and check each of the three profiles for
 ...
 ## Process For updating one profile from another
 
 Periodically, we move the contents of the `Current` profile to the `Previous` profile and move the contents of the `Work in Progress` profile to the `Current` profile
 
 These are the steps for doing that profile-contents move in the `~electricalcrew/PMRRM` directory. This is done occasionally when `Work in Progress` is considered stable and further development will be happening on it.
